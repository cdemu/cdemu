...
static gboolean __mirage_disc_cue_load_image (MIRAGE_Disc *self, gchar **filenames, GError **error) {
    ...
    /* Each CUE/BIN is one session, so we load all given filenames */
    for (i = 0; i < g_strv_length(filenames); i++) {
        ...
        /* There's slight problem with multi-session CUE images, namely that each
           CUE can be used independently... in order words, there's no way to determine
        ...
        /* Create CUE session */
        GObject *session = g_object_new(MIRAGE_TYPE_SESSION_CUE, NULL);
        __mirage_session_cue_set_cue_filename(MIRAGE_SESSION(session), filenames[i], NULL);
        ...
        __mirage_session_cue_finish_last_track(MIRAGE_SESSION(session), NULL);
