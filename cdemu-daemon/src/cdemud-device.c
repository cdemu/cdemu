...
static void __cdemud_device_delay_calculate (CDEMUD_Device *self, gint address, gint num_sectors) {
    ...
    if (_priv->dpm_emulation) {
        ...
        CDEMUD_DEBUG(self, DAEMON_DEBUG_DELAY, "%s: 0x%X->0x%X (%d): %f rotations\n", __debug__, _priv->current_sector, address, abs(_priv->current_sector - address), rotations);
...
static gint __cdemud_device_execute_command (CDEMUD_Device *self, CDEMUD_Command *cmd) {
    CDEMUD_DevicePrivate *_priv = CDEMUD_DEVICE_GET_PRIVATE(self);
...
static gboolean __cdemud_device_io_handler (GIOChannel *source, GIOCondition condition G_GNUC_UNUSED, gpointer data) {
    ...
    vres->tag = vreq->tag;
    vres->status = __cdemud_device_execute_command(self, &cmd);
...
static gpointer __cdemud_device_io_thread (gpointer data) {
    CDEMUD_Device *self = CDEMUD_DEVICE(data);
    CDEMUD_DevicePrivate *_priv = CDEMUD_DEVICE_GET_PRIVATE(self);
    ...
    _priv->main_context = g_main_context_new();
    _priv->main_loop = g_main_loop_new(_priv->main_context, FALSE);

    /* Create watch source */
    source = g_io_create_watch(_priv->io_channel, G_IO_IN);
    g_source_set_callback(source, (GSourceFunc)__cdemud_device_io_handler, self, NULL);
    g_source_attach(source, _priv->main_context);
...
gboolean cdemud_device_setup_mapping (CDEMUD_Device *self) {
    ...
    if (_priv->mapping_complete) {
        return _priv->mapping_complete;
    }

    /* Perform IOCTL */
    ioctl_ret = ioctl(g_io_channel_unix_get_fd(_priv->io_channel), 0xBEEF001, id);
    ...
    _priv->mapping_complete = complete;
    return complete;
}
...
gboolean cdemud_device_initialize (CDEMUD_Device *self, gint number, gchar *ctl_device, gchar *audio_driver, GError **error) {
    ...
    _priv->io_channel = g_io_channel_new_file(ctl_device, "r+", NULL);
    if (!_priv->io_channel) {
    ...
    _priv->io_thread = g_thread_create(__cdemud_device_io_thread, self, TRUE, NULL);
    if (!_priv->io_thread) {
...
static gboolean __cdemud_device_load_disc (CDEMUD_Device *self, gchar **filenames, GHashTable *options, GError **error) {
    CDEMUD_DevicePrivate *_priv = CDEMUD_DEVICE_GET_PRIVATE(self);
    gint media_type = 0;
    ...
    /* Load... */
    _priv->disc = libmirage_create_disc(filenames, _priv->disc_debug, options, error);
...
static GHashTable *__g_variant_to_g_hash_table (GVariant *variant)
{
    GHashTable *table;
    gint i;
...
gboolean cdemud_device_load_disc (CDEMUD_Device *self, gchar **filenames, GVariant *options, GError **error) {
    ...
    GHashTable *options_dict = __g_variant_to_g_hash_table(options);
    
    /* Load */
    g_mutex_lock(_priv->device_mutex);
    succeeded = __cdemud_device_load_disc(self, filenames, options_dict, error);
    g_mutex_unlock(_priv->device_mutex);    
...
gboolean cdemud_device_get_option (CDEMUD_Device *self, gchar *option_name, GVariant **option_value, GError **error) {
    ...
    *option_value = NULL;
    ...
    if (!g_strcmp0(option_name, "dpm-emulation")) {
        /* *** dpm-emulation *** */
        *option_value = g_variant_new("b", _priv->dpm_emulation);
    } else if (!g_strcmp0(option_name, "tr-emulation")) {
        /* *** tr-emulation *** */
        *option_value = g_variant_new("b", _priv->tr_emulation);
    } else if (!g_strcmp0(option_name, "device-id")) {
        /* *** device-id *** */
        *option_value = g_variant_new("(ssss)", _priv->id_vendor_id, _priv->id_product_id, _priv->id_revision, _priv->id_vendor_specific);
        ...
        /* *** library-debug-mask *** */
        gint mask;
        succeeded = mirage_debug_context_get_debug_mask(MIRAGE_DEBUG_CONTEXT(_priv->disc_debug), &mask, error);
        if (succeeded) {
            *option_value = g_variant_new("i", mask);
        }
    } else {
...
gboolean cdemud_device_set_option (CDEMUD_Device *self, gchar *option_name, GVariant *option_value, GError **error) {
    ...
    if (!g_strcmp0(option_name, "dpm-emulation")) {
        ...
        if (!g_variant_is_of_type(option_value, G_VARIANT_TYPE("b"))) {
            ...
            g_variant_get(option_value, "b", &_priv->dpm_emulation);
        }
    } else if (!g_strcmp0(option_name, "tr-emulation")) {
        /* *** tr-emulation *** */
        if (!g_variant_is_of_type(option_value, G_VARIANT_TYPE("b"))) {
            ...
            g_variant_get(option_value, "b", &_priv->tr_emulation);
        }
    } else if (!g_strcmp0(option_name, "device-id")) {
        /* *** device-id *** */
        ...
        if (!g_variant_is_of_type(option_value, G_VARIANT_TYPE("i"))) {
            ...
            gint mask;
            g_variant_get(option_value, "i", &mask);
            succeeded = mirage_debug_context_set_debug_mask(MIRAGE_DEBUG_CONTEXT(_priv->disc_debug), mask, error);
        }
    } else {
        /* Option not found */
        CDEMUD_DEBUG(self, DAEMON_DEBUG_WARNING, "%s: option '%s' not found; client bug?\n", __debug__, option_name);
...
static void __cdemud_device_finalize (GObject *obj) {
    ...
    /* Stop the main loop */
    if (_priv->main_loop) {
        if (g_main_loop_is_running(_priv->main_loop)) {
            g_main_loop_quit(_priv->main_loop);
            /* Wait for the thread to finish */
            g_thread_join(_priv->io_thread);
        }

        g_main_loop_unref(_priv->main_loop);
    }

    if (_priv->main_context) {
        g_main_context_unref(_priv->main_context);
    }

    if (_priv->io_channel) {
        g_io_channel_unref(_priv->io_channel);
    }
