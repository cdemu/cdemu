 ...
 *
 *  This code is based on GF(2^128) code from Linux kernel (found in
 *  lib/crypto/gf128mul.c), which in turn is based on Dr Brian Gladman's
 *  (GPL'd) work published at http://www.gladman.me.uk/AES/index.php
...
/* NOTE: since MDX/MDSv2 uses only LRW, which requires "bbe" variant,
 * only "be" variant of macro is kept here; the above notice is kept
 * as-is for the sake of completeness. */
#define xda_be(i) ( \
...
static const guint16 gf128mul_table_be[256] = gf128mul_dat(xda_be);

/* The following functions multiply a field element by x in the polynomial
 * field representation.  They use 64-bit word operations to gain speed,
 * but compensate for machine endianness, and hence work correctly on
 * both styles of machine. */
static inline guint64 gf128mul_mask_from_bit (guint64 x, int which)
{
    return ((gint64)(x << (63 - which)) >> 63);
}

static void gf128mul_x8_bbe (guint128_bbe *x)
{
    guint64 a = GUINT64_FROM_BE(x->a);
    guint64 b = GUINT64_FROM_BE(x->b);

    guint64 _tt = gf128mul_table_be[a >> 56];

    x->a = GUINT64_TO_BE((a << 8) | (b >> 56));
    x->b = GUINT64_TO_BE((b << 8) ^ _tt);
}

static inline void gf128mul_x_bbe (guint128_bbe *r, const guint128_bbe *x)
{
    guint64 a = GUINT64_FROM_BE(x->a);
    guint64 b = GUINT64_FROM_BE(x->b);

    guint64 _tt = gf128mul_mask_from_bit(a, 63) & 0x87;

    r->a = GUINT64_TO_BE((a << 1) | (b >> 63));
    r->b = GUINT64_TO_BE((b << 1) ^ _tt);
}

static inline void be128_xor (guint128_bbe *r, const guint128_bbe *p, const guint128_bbe *q)
{
    r->a = p->a ^ q->a;
    r->b = p->b ^ q->b;
}
...
gf128mul_64k_table *gf128mul_init_64k_table_bbe (const guint128_bbe *g)
{
    ...
    table->t[0][1] = *g;
    for (j = 1; j <= 64; j <<= 1) {
        gf128mul_x_bbe(&table->t[0][j + j], &table->t[0][j]);
    }

    for (i = 0;;) {
        for (j = 2; j < 256; j += j) {
            for (k = 1; k < j; ++k) {
                be128_xor(&table->t[i][j + k], &table->t[i][j], &table->t[i][k]);
            }
        }
...
{
    const guint8 *ap = (guint8 *)a;
    guint128_bbe r = table->t[0][ap[15]];
    for (guint i = 1; i < 16; ++i) {
        be128_xor(&r, &r, &table->t[i][ap[15 - i]]);
    }
    *a = r;
}
