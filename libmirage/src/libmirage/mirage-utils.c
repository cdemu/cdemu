 ...
 * </para>
 *
 * Returns: a new #MIRAGE_ParserInfo containing the provided information. It should
...
MIRAGE_ParserInfo *mirage_helper_create_parser_info (gchar *id, gchar *name, gchar *version, gchar *author, gboolean multi_file, gchar *description, gint num_suffixes, ...) {
    MIRAGE_ParserInfo *info = g_new0(MIRAGE_ParserInfo, 1);
    va_list args;
    gint i;
    ...
    info->multi_file = multi_file;
    
    info->description = g_strdup(description);
    
    va_start(args, num_suffixes); 

    info->suffixes = g_new0(gchar *, num_suffixes);
    for (i = 0; i < num_suffixes; i++) {
        info->suffixes[i] = g_strdup(va_arg(args, gchar *));
    }
 ...
 * @info: a #MIRAGE_ParserInfo to be destroyed
 *
 * <para>
 * A convenience function that properly destroys a #MIRAGE_ParserInfo structure,
 * freeing all its fields and the structure itself.
 * </para>
 **/
 ...
 * </para>
 *
 * Returns: a new #MIRAGE_FragmentInfo containing the provided information. It should
...
MIRAGE_FragmentInfo *mirage_helper_create_fragment_info (gchar *id, gchar *name, gchar *version, gchar *author, gchar *interface, gint num_suffixes, ...) {
    MIRAGE_FragmentInfo *info = g_new0(MIRAGE_FragmentInfo, 1);
    va_list args;
    gint i;
 ...
 * @info: a #MIRAGE_FragmentInfo to be destroyed
 *
 * <para>
 * A convenience function that properly destroys a #MIRAGE_FragmentInfo structure,
 * freeing all its fields and the structure itself.
 * </para>
 **/
