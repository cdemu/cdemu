...
static gboolean mirage_parser_ccd_sort_entries (MirageParserCcd *self, GError **error G_GNUC_UNUSED)
{
    GList *entry;
...
static gboolean mirage_parser_ccd_determine_track_mode (MirageParserCcd *self, GObject *track, GError **error)
{
    GObject *fragment;
    gint track_mode;
    guint8 *buffer;
...
static gboolean mirage_parser_ccd_build_disc_layout (MirageParserCcd *self, GError **error)
{
    ...
    G_LIST_FOR_EACH(entry, self->priv->entries_list) {
        ...
        if (ccd_cur_entry->Point == 0xA0) {
            ...
            mirage_fragment_iface_binary_main_data_set_stream(MIRAGE_FRAGMENT_IFACE_BINARY(fragment), self->priv->img_stream);
            ...
            mirage_fragment_iface_binary_subchannel_data_set_stream(MIRAGE_FRAGMENT_IFACE_BINARY(fragment), self->priv->sub_stream);
