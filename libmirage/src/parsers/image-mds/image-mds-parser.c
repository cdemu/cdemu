    ...
    for (gint i = 0; i < session_block->num_all_blocks; i++) {
        ...
        if (block->point > 0 && block->point < 99) {
            ...
            for (gint j = 0; j < block->number_of_files; j++) {
                ...
                /* Data stream */
                GInputStream *data_stream = mirage_parser_get_cached_data_stream(MIRAGE_PARSER(self), mdf_filename, error);
                if (!data_stream) {
                ...
                MirageFragment *fragment = mirage_contextual_create_fragment(MIRAGE_CONTEXTUAL(self), MIRAGE_TYPE_FRAGMENT_IFACE_BINARY, data_stream, error);
                if (!fragment) {
