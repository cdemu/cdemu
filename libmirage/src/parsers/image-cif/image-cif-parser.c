...
static gboolean mirage_parser_cif_read_descriptor (MirageParserCif *self, guint8 **data, guint16 *length, GError **error)
{
    ...
    /* Read entry length */
    if (g_input_stream_read(G_INPUT_STREAM(self->priv->cif_stream), &subblock_length, sizeof(subblock_length), NULL, NULL) != sizeof(subblock_length)) {
    ...
    if (g_input_stream_read(G_INPUT_STREAM(self->priv->cif_stream), subblock_data, subblock_length, NULL, NULL) != subblock_length) {
...
static GObject *mirage_parser_cif_parse_track_descriptor (MirageParserCif *self, guint8 *data, guint16 length G_GNUC_UNUSED, GError **error)
{
    ...
    /* Set file */
    if (!mirage_fragment_iface_binary_track_file_set_file(MIRAGE_FRAGMENT_IFACE_BINARY(fragment), self->priv->cif_filename, self->priv->cif_stream, error)) {
    ...
    mirage_fragment_iface_binary_track_file_set_offset(MIRAGE_FRAGMENT_IFACE_BINARY(fragment), offset_entry->offset);
    mirage_fragment_iface_binary_track_file_set_sectsize(MIRAGE_FRAGMENT_IFACE_BINARY(fragment), sector_size);
    if ((CIF_Track) descriptor->type == AUDIO) {
...
static gboolean mirage_parser_cif_parse_ofs_block (MirageParserCif *self, GError **error)
{
    ...
    if (g_input_stream_read(G_INPUT_STREAM(self->priv->cif_stream), &num_entries, sizeof(num_entries), NULL, NULL) != sizeof(num_entries)) {
    ...
    for (gint i = 0; i < num_entries && g_seekable_tell(G_SEEKABLE(self->priv->cif_stream)) < (self->priv->ofs_offset + self->priv->ofs_length); i++) {
        ...
        /* Read whole entry */
        if (g_input_stream_read(G_INPUT_STREAM(self->priv->cif_stream), &entry, sizeof(entry), NULL, NULL) != sizeof(entry)) {
