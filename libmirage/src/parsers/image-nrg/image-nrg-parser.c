...
static gboolean mirage_parser_nrg_load_session (MirageParserNrg *self, gint session_num, GError **error)
{
    ...
    if (self->priv->dao_header->mcn) {
        ...
        mirage_disc_set_mcn(self->priv->disc, self->priv->dao_header->mcn);
    }
    ...
    for (gint i = 0; i < self->priv->num_dao_blocks; i++) {
        ...
        */
        ...
        if (fragment_len) {
            ...
            fragment = mirage_contextual_create_fragment(MIRAGE_CONTEXTUAL(self), MIRAGE_TYPE_FRAGMENT_IFACE_BINARY, self->priv->nrg_stream, error);
            if (!fragment) {
            ...
            mirage_fragment_iface_binary_main_data_set_stream(MIRAGE_FRAGMENT_IFACE_BINARY(fragment), self->priv->nrg_stream);
