...
static gboolean mirage_parser_c2d_parse_track_entries (MIRAGE_Parser_C2D *self, GError **error)
{
    ...
    for (gint t = 0; t < num_tracks; t++) {
        ...
        /* Data fragment */
        GObject *data_fragment = libmirage_create_fragment(MIRAGE_TYPE_FRAG_IFACE_BINARY, self->priv->c2d_stream, G_OBJECT(self), error);
        if (!data_fragment) {
...
static GObject *mirage_parser_c2d_load_image (MIRAGE_Parser *_self, gchar **filenames, GError **error)
{
    ...
    /* Open file */
    self->priv->c2d_stream = libmirage_create_file_stream(filenames[0], G_OBJECT(self), error);
